...
def makeMap(centarr, title,folder):
    W = math.ceil(120 * boost)  # width of heatmap

    H = math.ceil(55 * boost)  # height of heatmap

    SCALE = 250  # increase scale to make heat map more blended
...
for filename in os.listdir("GameSheets"):
    ...
    # print result
    print("Game info for game on {year}-{month}-{day} where {home} was home vs visiting {away}".format(year=gameinfo[0],
                                                                                                       month=gameinfo[1],
                                                                                                       day=gameinfo[2],
                                                                                                       away=gameinfo[3],
                                                                                                       home=gameinfo[4]))
    ...
    with open('GameSheets/' + filename) as f:
        ...
        try:
            """if teamKey == gameinfo[3] or teamKey == gameinfo[4]:
                ...
                for line in eventArray:
                    ...
                    if prevcode == 20 and event['t'] == 20 and thrower in throwDict:
                        throwDictDF[thrower].append((event['y'] - prevy, "c", gameinfo[3], gameinfo[4]))
                        throwDict[thrower].append(
                            (math.hypot(event['x'] - prevx, event['y'] - prevy), 'c', gameinfo[3], gameinfo[4]))
                        throwDictCF[thrower].append((event['x'] - prevx, "c", gameinfo[3], gameinfo[4]))

                    elif prevcode == 20 and event['t'] == 20:
                        throwDictDF[thrower] = [(event['y'] - prevy, "c", gameinfo[3], gameinfo[4])]
                        throwDictCF[thrower] = [(event['x'] - prevx, "c", gameinfo[3], gameinfo[4])]
                        throwDict[thrower] = [
                            (math.hypot(event['x'] - prevx, event['y'] - prevy), "c", gameinfo[3], gameinfo[4])]

                    if event['t'] == 8 and prevcode == 20:
                        if thrower in throwDict:
                            throwDictDF[thrower].append((event['y'] - prevy, 't', gameinfo[3], gameinfo[4]))
                            throwDictCF[thrower].append((event['x'] - prevx, 't', gameinfo[3], gameinfo[4]))
                            throwDict[thrower].append(
                                (math.hypot(event['x'] - prevx, event['y'] - prevy), 't', gameinfo[3], gameinfo[4]))
                        else:
                            throwDictDF[thrower] = [(event['y'] - prevy, "t", gameinfo[3], gameinfo[4])]
                            throwDictCF[thrower] = [(event['x'] - prevx, "t", gameinfo[3], gameinfo[4])]
                            throwDict[thrower] = [
                                (math.hypot(event['x'] - prevx, event['y'] - prevy), "t", gameinfo[3], gameinfo[4])]
...
"""
            ...
            if teamKey == gameinfo[3] or teamKey == "ALL":
                ...
                for line in eventArray:
                    ...
                    if prevcode == 20 and event['t'] == 20 and thrower in throwDict:
                        throwDictDF[thrower].append((event['y'] - prevy, "c",gameinfo[3],gameinfo[4]))
                        throwDict[thrower].append((math.hypot(event['x'] - prevx, event['y'] - prevy), 'c',gameinfo[3],gameinfo[4]))
                        throwDictCF[thrower].append((event['x'] - prevx, "c", gameinfo[3], gameinfo[4]))

                    elif prevcode == 20 and event['t'] == 20:
                        throwDictDF[thrower] = [(event['y'] - prevy, "c",gameinfo[3],gameinfo[4])]
                        throwDictCF[thrower] = [(event['x'] - prevx, "c", gameinfo[3], gameinfo[4])]
                        throwDict[thrower] = [(math.hypot(event['x'] - prevx, event['y'] - prevy), "c",gameinfo[3],gameinfo[4])]
                    ...
                    if event['t'] == 8 and prevcode == 20:
                        if thrower in throwDict:
                            throwDictDF[thrower].append((event['y'] - prevy, 't',gameinfo[3],gameinfo[4]))
                            throwDictCF[thrower].append((event['x'] - prevx, 't', gameinfo[3], gameinfo[4]))
                            throwDict[thrower].append((math.hypot(event['x'] - prevx, event['y'] - prevy), 't',gameinfo[3],gameinfo[4]))
                        else:
                            throwDictDF[thrower] = [(event['y'] - prevy, "t",gameinfo[3],gameinfo[4])]
                            throwDictCF[thrower] = [(event['x'] - prevx, "t", gameinfo[3], gameinfo[4])]
                            throwDict[thrower] = [(math.hypot(event['x'] - prevx, event['y'] - prevy), "t",gameinfo[3],gameinfo[4])]
                    ...
                    prevcode = event['t']
                if teamKey != "ALL":
                    makeMap(centerArrGame, str(teamKey)+"_vs_"+gameinfo[4]+"_All_Games_For_"+str(evCode)+"_code",teamKey)
            ...
            if teamKey == gameinfo[4] or teamKey == "ALL":
                ...
                for line in eventArray:
                    ...
                    if prevcode == 20 and event['t'] == 20 and thrower in throwDict:
                        throwDictDF[thrower].append((event['y'] - prevy, "c",gameinfo[4],gameinfo[3]))
                        throwDictCF[thrower].append((event['x'] - prevx, "c", gameinfo[4], gameinfo[3]))
                        throwDict[thrower].append((math.hypot(event['x'] - prevx, event['y'] - prevy), 'c',gameinfo[4],gameinfo[3]))

                    elif prevcode == 20 and event['t'] == 20:
                        throwDictDF[thrower] = [(event['y'] - prevy, "c",gameinfo[4],gameinfo[3])]
                        throwDictCF[thrower] = [(event['x'] - prevx, "c", gameinfo[4], gameinfo[3])]
                        throwDict[thrower] = [(math.hypot(event['x'] - prevx, event['y'] - prevy), "c",gameinfo[4],gameinfo[3])]
                    ...
                    if event['t'] == 8 and prevcode == 20:
                        if thrower in throwDict:
                            throwDictDF[thrower].append((event['y'] - prevy, 't',gameinfo[4],gameinfo[3]))
                            throwDictCF[thrower].append((event['x'] - prevx, 't', gameinfo[4], gameinfo[3]))
                            throwDict[thrower].append((math.hypot(event['x'] - prevx, event['y'] - prevy), 't',gameinfo[4],gameinfo[3]))
                        else:
                            throwDictDF[thrower] = [(event['y'] - prevy, "t",gameinfo[4],gameinfo[3])]
                            throwDictCF[thrower] = [(event['x'] - prevx, "t", gameinfo[4], gameinfo[3])]
                            throwDict[thrower] = [(math.hypot(event['x'] - prevx, event['y'] - prevy), "t",gameinfo[4],gameinfo[3])]
                ...
                if teamKey != "ALL":
                    makeMap(centerArrGame, str(teamKey)+"_vs_"+gameinfo[3]+"_All_Games_For_"+str(evCode)+"_code",teamKey)
...
if len(centerArrIndi) > 0:
    makeMap(centerArrIndi, "All_Games_For_Player_"+str(playerCode),teamKey)
makeMap(centerArrTot, str(teamKey)+"_All_Games_For_"+str(evCode)+"_code",teamKey)
...
with open('ALL\playDir.csv', 'w', newline='') as csvfile:
    ...
    for key in playerDict:
        playerWriter.writerow([key,playerDict[key][0],playerDict[key][1],playerDict[key][0]+" "+playerDict[key][1],playerDict[key][2]])
